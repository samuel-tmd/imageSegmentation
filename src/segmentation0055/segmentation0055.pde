...
void draw() {
  PrintWriter output;
  PImage img = loadImage("0055.jpg"); 
  PImage imgGT = loadImage("GT0055.png"); 
  output = createWriter("results0055.csv");
...
// Filtro de média
      ...
      for (int i = -10; i <= 10; i++) {
        for (int j = -10; j <=10; j++) {
          ...
          if (newX >= 0 && newX < img.width && newY >= 0 && newY < img.height) {
            media += red(img.pixels[newY * img.width + newX])+20;
            qtde++;
          }
        }
      }
  ...
  for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) { 
      ...
      if(x < 80 || x > 250 || y < 0) segBB.pixels[pos] = color(0);
    }
  }
  ...
  // Salvando as imagens
  image(segSobel, 0, 0);
  save("segSobel_0055.jpg");

  image(segMedia, 0, 0);
  save("segMedia_0055.jpg");
  
  image(segLim, 0, 0); 
  save("segLim_0055.jpg");
  
  image(segBB, 0, 0); 
  save("segBB_0055.jpg");

  image(segImg, 0, 0); 
  save("segImg_0055.jpg");
}
